---
name: my-websearch
description: Single entry skill for my-websearch setup and focused live retrieval, preferring local CLI/daemon paths while remaining compatible with workspace-exposed MCP tools.
version: 1.4.0
...
Use this as the single user-facing entry skill for `my-websearch`.

Assumption:
- The preferred low-friction path is a working local `my-websearch` CLI/daemon setup.
- A workspace that already exposes the `my-websearch` MCP tools such as `search`, `fetchWebContent`, and `fetchGithubReadme` is also a valid path and should continue to work.
- If neither path is available, treat that as a missing `my-websearch` capability in the current workspace, not as a broken skill.
...
1. First determine whether `my-websearch` is already usable through a local CLI/daemon path or through workspace-exposed MCP tools.
...
5. Treat `my-websearch --help` as the primary CLI reference. When command names, daemon flags, spawn behavior, or action parameters are unclear, check `--help` before guessing.
...
5. Perform the chosen path only after the required inputs and confirmations are in place.
   - local CLI/daemon mode when the runtime can launch `my-websearch` directly
   ...
   - existing HTTP endpoint mode when the user already has a reachable `my-websearch` server
...
9. When the goal is to start or validate the local daemon path, use explicit commands: `my-websearch serve` to start it and `my-websearch status` to check it. Do not treat bare `my-websearch` as the recommended daemon start command.
10. During setup, when package installation is required, ask about proxy or npm mirror needs before long-running install steps in restricted networks. If installation repeatedly hangs, times out, or fails on package download, treat that as an environment or network issue first, not as an `my-websearch` core failure.
11. If the next step after daemon startup is expected to perform live network actions such as `search`, `fetch-web`, or other public-page retrieval, ask about runtime proxy needs before starting `my-websearch serve`. If the goal is only minimal local validation such as `serve` followed by `status`, runtime proxy can wait until a real networked action is planned.
...
- For agent automation, prefer explicit commands: `my-websearch serve` for daemon startup, `my-websearch status` for daemon checks, and one-shot commands such as `my-websearch search ...` or `my-websearch fetch-web ...` for direct actions.
- If CLI behavior is unclear, or if command names or flags may have changed, consult `my-websearch --help` first and follow the current help output rather than relying on memory.
...
- Keep npm proxy or registry guidance separate from runtime proxy guidance: npm proxy or mirror settings help package installation, while runtime proxy settings affect `my-websearch serve` and the networked search/fetch actions that follow it.
...
- If an error mentions `browserType.launch`, `Executable doesn't exist`, `Playwright client is not available`, or a missing Chromium executable, treat it first as missing browser dependency or browser configuration, not as a generic `my-websearch` core failure.
...
1. State that the missing capability is usable `my-websearch` access in the current workspace, either through local CLI/daemon or through MCP integration.
2. State what cannot be done yet: live web search, page fetch, and GitHub README retrieval through `my-websearch`.
3. State that the skill itself is still fine; the current workspace just is not exposing a usable `my-websearch` path yet.
...
- Validate whether the current runtime now exposes a usable `my-websearch` path and core tools.
